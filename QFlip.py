...
# Save path
SAVE_FILE_NAME = f"QFlip_{LOAD_PATH_Q_AWARE[-8:]}_{datetime.datetime.now().strftime('%Y-%m-%d')}.csv"
        ...
        for key in keys_list:
            ...
            for bit_position in set_values_bits.tolist():
                for n_flips in range(1, N_FLIPS_LIMIT + 1):
                    ...
                    main_file.flush()
                    file_idx += 1
                    print(f"Sim={simulation_number} Model={file_idx} layer={key} flips={n_flips} bit-pos={bit_position} iter-time={datetime.timedelta(seconds = time.time() - iteration_time)} time-now={datetime.timedelta(seconds = time.time() - total_time)}\n")
