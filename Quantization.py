...
def predict_tflite(interpreter: tf.lite.Interpreter, test_images : np.ndarray, verbose : int = 0) -> npt.NDArray[np.float32]:
    """ Evaluate TFLite Model:
    - Receives the interpreter and returns the prediction output.
    """
    ...
    for i, test_image in enumerate(test_images):
        ...
        prediction.append(np.copy(output()[0]))
...
def split_model_full(
    q_aware_model : Functional | tf.keras.Model, 
    q_model_info : QuantizedModelInfo, 
    split_key_index : int = 0, 
    tflite_output : bool = False) -> List[tf.keras.Model | Functional | tf.lite.Interpreter | None]:
    """ Divides the model in 2
    - First part model with either quantized or non-quantized weights
    - Second part model with dequantized weights (floating point)
    - Assumes selected start_index is always an index next to one of a convolutional layer index!
    ...
    # Parameters model 1
    layers : Dict[str, List[Dict[str, int | str | Dict | List]]] = dict()
    section_index = q_model_info.layers_indexes[q_model_info.keys.index(section_keys[split_key_index])]
    layers["first part"] = copy.deepcopy(configuration['layers'][0: section_index])
    layers["middle part"] = copy.deepcopy(configuration['layers'][section_index: section_index + 1])
    layers["second part"] = copy.deepcopy(configuration['layers'][section_index + 1:])

    first_part_config = {
        'name' : 'model_part_1',
        'trainable' : True,
        'layers' : layers["first part"],
        'input_layers' : [[layers["first part"][0]['name'], 0, 0]],
        'output_layers' : [[layers["first part"][-1]['name'], 0, 0]],
    }

    models : List[Functional | tf.keras.Sequential | tf.lite.Interpreter] = []
    with tfmot.quantization.keras.quantize_scope():
        models.append(tf.keras.models.Model.from_config(first_part_config))

    layers["middle part"] = unwrapp_layers(layers["middle part"])
    layers["middle part"][-1]['config']['activation'] = 'linear'
    layers["middle part"][-1]['config']['use_bias'] = False
    
     # Add input layer to the middle part model
    input_config = copy.deepcopy(layers["first part"][0])
    input_config['config']['batch_input_shape'] = models[0].output_shape
    input_config['name'] = 'input_middle'
    input_config['config']['name'] = input_config['name']
    
    layers["middle part"][0]['inbound_nodes'] = [[[input_config['name'], 0, 0, {}]]]
    layers["middle part"] = [input_config] + layers["middle part"]

    middle_part_config = {
        'name' : 'model_middle_part',
        'trainable' : True,
        'layers' : layers["middle part"],
        'input_layers' : [[layers["middle part"][0]['name'], 0, 0]],
        'output_layers' : [[layers["middle part"][-1]['name'], 0, 0]],
    }

    with tfmot.quantization.keras.quantize_scope():
        # m1 = tf.keras.models.model_from_config(configuration)
        models.append(tf.keras.models.Model.from_config(middle_part_config))

    if layers["second part"]:
        # Add input layer to the second part model
        input_config = copy.deepcopy(layers["first part"][0])
        input_config['config']['batch_input_shape'] = models[1].output_shape
        input_config['name'] = 'input_2'
        input_config['config']['name'] = input_config['name']
        
        quantization_config = copy.deepcopy(configuration['layers'][1])
        layers["second part"][0]['inbound_nodes'][0][0][0] = quantization_config['name']
        quantization_config['inbound_nodes'][0][0][0] = input_config['name']
        layers["second part"] = [input_config, quantization_config] + layers["second part"]
        
        second_part_config = {
            'name' : 'model_part_2',
            'trainable' : True,
            'layers' : layers["second part"],
            'input_layers' : [[layers["second part"][0]['name'], 0, 0]],
            'output_layers' : [[layers["second part"][-1]['name'], 0, 0]],
        }

        with tfmot.quantization.keras.quantize_scope():
            models.append(tf.keras.models.Model.from_config(second_part_config))
    else:
        models.append(None)

    # Weights extraction
    weights = [q_aware_model.layers[idx].get_weights() for idx in range(len(q_aware_model.layers))]
    first_weights = copy.deepcopy(weights[: section_index])
    middle_weights = copy.deepcopy(weights[section_index: section_index + 1])
    second_weights = copy.deepcopy(weights[section_index + 1:])

    # Preparing weights for the first part (does not need preparation)

    # Preparing weights for the middle part
    try:
        idx = q_model_info.layers_indexes.index(section_index)
    except:
        raise Exception("Index of middle layer not found")
    else:
        key = q_model_info.keys[idx]
        middle_weights[0] = [q_model_info.quantized_weights[key]]
    middle_weights = [[]] + middle_weights

    # Preparing weights for the second part
    second_weights = [[]] + second_weights
    second_weights.insert(1, [weights[section_index][-2], weights[section_index][-1], -1])

    # Weights assignation
    for idx in range(len(models[0].layers)):
        models[0].layers[idx].set_weights(first_weights[idx])

    for idx in range(len(models[1].layers)):
        models[1].layers[idx].set_weights(middle_weights[idx])

    if layers["second part"]:
        for idx in range(len(models[2].layers)):
            models[2].layers[idx].set_weights(second_weights[idx])
    ...
    if tflite_output:
        # Conversion to TF Lite model
        converter_1 = tf.lite.TFLiteConverter.from_keras_model(models[0])
        converter_1.optimizations = [tf.lite.Optimize.DEFAULT]
        tflite_1 = converter_1.convert()
        interpreter_1 = tf.lite.Interpreter(model_content = tflite_1)
        # # Save TFLite model
        # with open("./model/tflite_pt1.tflite", 'wb') as output_file:
        #     output_file.write(tflite_1)

        if layers["second part"]:
            # Conversion to TF Lite model
            converter_2 = tf.lite.TFLiteConverter.from_keras_model(models[2])
            converter_2.optimizations = [tf.lite.Optimize.DEFAULT]
            tflite_2 = converter_2.convert()
            interpreter_2 = tf.lite.Interpreter(model_content = tflite_2)
            # # Save TFLite model
            # with open("./model/tflite_pt2.tflite", 'wb') as output_file:
            #     output_file.write(tflite_2)
        else:
            interpreter_2 = None

        models[0] = interpreter_1
        models[2] = interpreter_2
...
def model_full_predict_by_batches(data_input : npt.NDArray[np.int32], 
test_labels : npt.NDArray[np.uint8],
n_partitions : int, 
q_model_info : QuantizedModelInfo,
models : List[tf.keras.Model | tf.lite.Interpreter | None] = None,
split_key_index : int = 0) -> Tuple[npt.NDArray[np.int32], float, float]:
    """ Evaluates both parts of the model and deletes memory unused
    - Different behaviour depending if you selected different evaluation_mode:
        * manual_saturation : Implements a manual floating-point saturation to output of model 1 before evaluating the second model
        * no_input_saturation : Feeds the output of model 1 directly to model 2
    ...
    if models[0] is not None:
        if isinstance(models[0], tf.lite.Interpreter):
            partial_out = predict_tflite(models[0], data_input) 
        if isinstance(models[0], tf.keras.Model):
            partial_out = models[0].predict(data_input) 
        # Output has to be quantized!
        quantized_input = np.round(partial_out/q_model_info.input_scales[key]).astype(int)

    # Batches prediction for the middle model
    quant_conv : List[npt.NDArray[np.float32]] | npt.NDArray[np.int32] = []
    quant_processed : List[npt.NDArray[np.int32]] | npt.NDArray[np.int32] = []
    for i in range(n_iterations):
        batch_quant_conv : npt.NDArray[np.float32] = models[1].predict(quantized_input[i*BATCH_SIZE: (i + 1)*BATCH_SIZE]) # np.float32
        quant_conv.append(batch_quant_conv)
        if models[2] is not None:
            batch_quant_processed : npt.NDArray[np.int32] = tf.nn.relu(tf.nn.bias_add(batch_quant_conv, q_model_info.quantized_biases[key])).numpy().astype(int)
        else:
            batch_quant_processed : npt.NDArray[np.int32] = tf.nn.bias_add(batch_quant_conv, q_model_info.quantized_biases[key]).numpy().astype(int)

        quant_processed.append(batch_quant_processed)

    # Garbage collection
    del batch_quant_conv # 351.5626 MBi Numpy array
    del batch_quant_processed # 351.5626 MBi Numpy array
    garbage_collection()

    if quant_conv:
        quant_conv = np.concatenate(quant_conv).astype(np.int32) # 703.12515 MBi Numpy array. Important it must be int32 for flipping values later and avoiding rounding error when using float32
    quant_processed = np.concatenate(quant_processed) # 703.125 MBi Numpy array

    dequant_processed : npt.NDArray[np.float32] = (q_model_info.bias_scales[key] * quant_processed).astype(np.float32)
    rescaled_processed = (q_model_info.output_scales[key] * np.round(dequant_processed / q_model_info.output_scales[key])).astype(np.float32)
    
    # Garbage collection
    del quant_processed # 703.125 MBi Numpy array
    del dequant_processed # 703.125 MBi Numpy array
    garbage_collection()
    
    # Saturation
    rescaled_processed[rescaled_processed >= q_model_info.dequantized_output_max[key]] = q_model_info.dequantized_output_max[key]
    rescaled_processed[rescaled_processed <= q_model_info.dequantized_output_min[key]] = q_model_info.dequantized_output_min[key]

    if models[2] is not None:
        if isinstance(models[2], tf.lite.Interpreter):
            loss, accuracy = evaluate_tflite(models[2], rescaled_processed, test_labels, verbose = 0)
        if isinstance(models[2], tf.keras.Model):
            loss, accuracy = models[2].evaluate(rescaled_processed, test_labels, verbose = 0)
    else:
        # It means it is the last layer so we need to apply the activation function manually
        predictions = tf.nn.softmax(rescaled_processed).numpy()
    ...
    # Garbage collection
    del rescaled_processed # 703.125 MBi Numpy array
    garbage_collection()

    return quant_conv, loss, accuracy

def model_predict_corrupted_convolution(data_input : npt.NDArray[np.int32], 
test_labels : npt.NDArray[np.uint8],
n_partitions : int, 
q_model_info : QuantizedModelInfo,
models : List[tf.keras.Model | tf.lite.Interpreter | None] = None,
