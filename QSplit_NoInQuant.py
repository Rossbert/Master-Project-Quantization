...
SAVE_FILE_NAME = f"Quant_Split_NoInQuant_{datetime.datetime.now().strftime('%Y-%m-%d_%H')}.csv"
...
quant_conv1, test_loss, test_accuracy = Quantization.prediction_by_batches(
    ...
    evaluation_mode = Quantization.ModelEvaluationMode.no_input_saturation, 
    start_index = FIRST_MAX_POOL_INDEX,
    )
